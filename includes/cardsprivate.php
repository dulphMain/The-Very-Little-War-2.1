...
if($tuto['niveaututo'] < 10) { 
    ...
    elseif($tuto['niveaututo'] == 4) {
        ...
        <center>'.nombreEnergie(floor($ressources['energie']).'/'.$placeDepot.' <span style="color:green;margin-left:10px"> +'.chiffrePetit(revenuEnergie($constructions['generateur'],$_SESSION['login'])).'/h').'</span></center> <br>signifie que vous possédez <strong>'.floor($ressources['energie']).'</strong> sur les <strong>'.$placeDepot.'</strong> que vous pouvez posséder au maximum. De plus vous produisez <strong>'.revenuEnergie($constructions['generateur'],$_SESSION['login']).'</strong> d\'énergie par heure.<br><br><hr><br>
